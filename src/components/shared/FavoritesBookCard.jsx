...
function FavoritesBookCard({ book, onRemove, onAddToCart }) {
  return (
    <div className="relative bg-white rounded-xl shadow-sm flex flex-col">
      <button
        onClick={() => onRemove(book.id)}
        aria-label="حذف من المفضلة"
        className="absolute top-2 left-2 w-8 h-8 bg-white rounded-md shadow flex items-center justify-center text-red-600 hover:bg-red-600 hover:text-white transition-colors duration-200"
      >
        <LuTrash />
      </button>
